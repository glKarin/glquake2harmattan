...
ifeq ($(ARCH),arm)
RELEASE_CFLAGS=$(BASE_CFLAGS) -ffast-math -funroll-loops \
	-fomit-frame-pointer -fexpensive-optimizations -g -O2 -Wall
endif
...
BASE_CFLAGS=-Wall -pipe -Dstricmp=strcasecmp -D_HARMATTAN -D_HARMATTAN_MULTITOUCH \
						-D_HARMATTAN_VER='\"harmattan2015-1-gles\"' \
						-D_HARMATTAN_APPNAME='\"Quake 2 Harmattan\"' \
						-D_HARMATTAN_DEV='\"karin\"' \
						-D_HARMATTAN_DEVCODE='\"Natasha\"' \
						-D_HARMATTAN_DESC='\"Quake 2 + Touch for OpenGLES 1.1 on MeeGo Harmattan\"' \
						-D_HARMATTAN_RELEASE='\"20150824\"' \
						-D_HARMATTAN_RESC='\"/usr/share/quake2/resc/\"'
ifeq ($(HAVE_IPV6),YES)
...
DEBUG_CFLAGS=$(BASE_CFLAGS) -g # -D_HARMATTAN_DEBUG

ifeq ($(OSTYPE),FreeBSD)
LDFLAGS=-lm
endif
ifeq ($(OSTYPE),Linux)
LDFLAGS=$(shell sdl-config --libs) -lm -ldl
endif
...
GLXLDFLAGS=-L/usr/X11R6/lib$(_LIB) -lX11 -lXext -lEGL # -lXxf86dga -lXxf86vm
...
REF_SOFT_X11_OBJS = \
	...
	$(BUILDDIR)/karin/vkb.o \
	$(BUILDDIR)/karin/swr_x11_compatibility.o

REF_SOFT_SDL_OBJS = \
	$(BUILDDIR)/ref_soft/rw_linux.o \
	$(BUILDDIR)/ref_soft/rw_sdl.o \
	$(BUILDDIR)/karin/swr_sdl.o \
	$(BUILDDIR)/karin/vkb.o \
	$(BUILDDIR)/karin/sdl_vkb.o
...
$(BUILDDIR)/ref_softsdl.$(SHLIBEXT) : $(REF_SOFT_OBJS) $(REF_SOFT_SDL_OBJS)
	$(CC) $(CFLAGS) $(SHLIBLDFLAGS) -o $@ $(REF_SOFT_OBJS) \
		$(REF_SOFT_SDL_OBJS) $(SDLLDFLAGS) -lSDL_image
...
$(BUILDDIR)/karin/sdl_vkb.o :      $(KARIN_DIR)/sdl_vkb.c $(KARIN_DIR)/sdl_vkb.h $(KARIN_DIR)/vkb_general.c
...
$(BUILDDIR)/karin/swr_x11_compatibility.o :       $(KARIN_DIR)/swr_x11_compatibility.c
	$(DO_SHLIB_CC) $(XCFLAGS)
...
$(BUILDDIR)/karin/swr_sdl.o :       $(KARIN_DIR)/swr_sdl.c $(KARIN_DIR)/win_general.c
	$(DO_SHLIB_CC) $(SDLCFLAGS)
...
REF_GL_OBJS = \
	...
	$(BUILDDIR)/karin/q3_png.o \
	$(BUILDDIR)/karin/gl_vkb1.o \
	$(BUILDDIR)/karin/vkb.o

REF_GLX_OBJS = \
	$(BUILDDIR)/ref_gl/gl_glx.o \
	$(BUILDDIR)/karin/gl_egl1.o \
	$(BUILDDIR)/karin/m_xi2.o \
...
$(BUILDDIR)/karin/gl_egl1.o :         $(KARIN_DIR)/gl_egl1.c $(KARIN_DIR)/gl_egl.c $(KARIN_DIR)/gl_egl.h $(KARIN_DIR)/win_general.c  
	$(DO_GL_SHLIB_CC) $(GLXCFLAGS)

$(BUILDDIR)/karin/m_xi2.o :         $(KARIN_DIR)/m_xi2.c $(KARIN_DIR)/m_xi2.h 
	$(DO_GL_SHLIB_CC) $(GLXCFLAGS)

$(BUILDDIR)/karin/q3_png.o :         $(KARIN_DIR)/q3_png.c $(KARIN_DIR)/q3_png.h 
	$(DO_GL_SHLIB_CC)

$(BUILDDIR)/karin/gl_vkb1.o :         $(KARIN_DIR)/gl_vkb1.c $(KARIN_DIR)/gl_vkb.h $(KARIN_DIR)/vkb_general.c 
	$(DO_GL_SHLIB_CC)
